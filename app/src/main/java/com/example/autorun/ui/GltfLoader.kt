 ...
 * GLB(glTF 2.0)ファイルを解析し、頂点、インデックス、カラー、法線情報を抽出します。
 */
object GltfLoader {
    ...
    fun loadGlb(context: Context, fileName: String): ModelData? {
        return try {
            ...
            if (nodes != null && meshes != null) {
                for (i in 0 until nodes.length()) {
                    ...
                    if (node.has("mesh")) {
                        ...
                        processMesh(json, binBuffer, meshes.getJSONObject(meshIdx), matrix, allVertices, allIndices, allColors, allNormals)
                    }
                }
            } else if (meshes != null) {
                for (i in 0 until meshes.length()) {
                    processMesh(json, binBuffer, meshes.getJSONObject(i), identity, allVertices, allIndices, allColors, allNormals)
                }
            }

            val vArray = allVertices.toFloatArray()
            val nArray = allNormals.toFloatArray()
            ...
            Log.i("GltfLoader", "Loaded: $fileName, Verts: ${vArray.size/3}")
            return ModelData(vArray, allIndices.toShortArray(), allColors.toFloatArray(), nArray)
        } catch (e: Exception) {
    ...
    private fun processMesh(json: JSONObject, binBuffer: ByteBuffer, mesh: JSONObject, matrix: FloatArray, allVertices: MutableList<Float>, allIndices: MutableList<Short>, allColors: MutableList<Float>, allNormals: MutableList<Float>) {
        ...
        for (p in 0 until primitives.length()) {
            ...
            if (attributes.has("NORMAL")) {
                ...
                // 法線もノードの回転を反映させる（平行移動は無視）
                val normalMatrix = matrix.clone()
                normalMatrix[12]=0f; normalMatrix[13]=0f; normalMatrix[14]=0f
                for (i in nData.indices step 3) {
                    val res = FloatArray(4)
                    Matrix.multiplyMV(res, 0, normalMatrix, 0, floatArrayOf(nData[i], nData[i+1], nData[i+2], 0f), 0)
                    allNormals.add(res[0]); allNormals.add(res[1]); allNormals.add(res[2])
                }
            } else {
                for (i in 0 until vData.size / 3) { allNormals.add(0f); allNormals.add(1f); allNormals.add(0f) }
            }

            if (primitive.has("indices")) {
                val iData = getIndicesArray(json, binBuffer, primitive.getInt("indices"))
                for (idx in iData) allIndices.add((idx + baseVertexIdx).toShort())
            } else {
            ...
            if (attributes.has("COLOR_0")) {
                ...
                for (i in 0 until vData.size / 3) { allColors.add(0.7f); allColors.add(0.7f); allColors.add(0.7f); allColors.add(1.0f) }
            }
        }
    }
    ...
    private fun getFloatArray(json: JSONObject, binBuffer: ByteBuffer, accessorIdx: Int): FloatArray {
        ...
        val stride = bufferView.optInt("byteStride", 0).let { if (it == 0) num * 4 else it }
